...
export class ProductService {
    ...
    getProducts(): Observable<IProduct[]> {
        ...
        return this.http.get<IProduct[]>(this.productsUrl)
                        .pipe(
                            tap(data => console.log('Products Data:', JSON.stringify(data))),
    ...
    getProduct(id: number): Observable<IProduct> {
        ...
        return this.http.get<IProduct>(url)
                        .pipe(
                            tap(data => console.log('Data reretrieved: ' + JSON.stringify(data))),
                            catchError(this.handleError)
                        );
    ...
    deleteProduct(id: number): Observable<IProduct> {
        ...
        return this.http.delete<IProduct>(url, { headers: headers} )
                        .pipe(
                            ...
                            tap(data => {
                                // Find the item index and delete it from the products array
                                const foundItemIdx = this.products.findIndex(item => item.id === id);
                                if (foundItemIdx > -1) {
                                    this.products.splice(foundItemIdx, 1);
                                }
                            }),
                            catchError(this.handleError)
                        );
